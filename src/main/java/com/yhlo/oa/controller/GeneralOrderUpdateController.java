...
@Slf4j
@FXMLController
public class GeneralOrderUpdateController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        loginName = userPreferences.get("loginName","");
       ...
       // NumberTableCellFactory.createNumberColumn(mColLineItem, 1);
        ...
        Callback<TableColumn<GeneralOrderDetailVO, String>, TableCell<GeneralOrderDetailVO, String>> materialCellFactory
                = (TableColumn<GeneralOrderDetailVO, String> p) -> new EditOrderDetailCellPublicComboBox("showName","物料",xiaoszz,"updatePage");
            ...
            row.setOnMouseClicked(event -> {
                ...
                if((!row.isEmpty()) && event.getClickCount()==1 && name.equals(MouseButton.PRIMARY.name())){
                    ...
                    Callback<TableColumn<GeneralOrderDetailVO, String>, TableCell<GeneralOrderDetailVO, String>> factoryCellFactory
                            = (TableColumn<GeneralOrderDetailVO, String> p) -> new EditOrderDetailCellPublicComboBox("name1","工厂",xiaoszz+"|"+rowData.getMaterialNo(),"updatePage");
        ...
        Callback<TableColumn<GeneralOrderDetailVO, String>, TableCell<GeneralOrderDetailVO, String>> lineItemTypeCellFactory
                = (TableColumn<GeneralOrderDetailVO, String> p) -> new EditOrderDetailCellPublicComboBox("tkey","行项目类型",xiaoszz,"updatePage");
    ...
    public void ipmortDetail(ActionEvent actionEvent) {
        ...
        if (file != null) {
            ...
            try {
                ...
                String errorMessage = "";
                if(null != errorList && errorList.size()>0){//存在未导入成功的数据时，凭拼接报错信息
    ...
    public void editData(ActionEvent actionEvent) {
        ...
        String approvalStatus = (String) approvalStatusAssembly.getSelectionModel().getSelectedItem();
        String deliveryStatus = (String) deliveryStatusAssembly.getSelectionModel().getSelectedItem();
        String ddsSalesOrderNo = ddsSalesOrderNoAssembly.getText();

        if(!OrderStatuEnum.ORDER_STATU_PENDING.getName().equals(approvalStatus)){
            CommonUtil._alertInformation("只能修改审批状态为《待审批》的订单，请检查！！！");
            return;
        }

        List<GeneralOrderVO> orderStatusList = orderService.getOrderStatus(ddsSalesOrderNo);
        if(null != orderStatusList && orderStatusList.size()>0 &&
                !OrderStatuEnum.ORDER_STATU_PENDING.getName().equals(orderStatusList.get(0).getApprovalStatus())){//二次校验，避免打开的窗口没有刷新，导致没有校验到
            CommonUtil._alertInformation("只能修改审批状态为《待审批》的订单，请刷新后重新查看该订单状态是否满足！！！");
            return;
        }
        ...
        go.setCreateBy(userName);
        go.setUpdateBy(userName);

        log.info(userName+"在"+DateUtils.getTime()+"执行了<修改订单>操作，操作数据："
                +"主数据【"+go+"】\n"+"明细数据【"+detailList+"】");

        GeneralOrderHistoryVO orderHis = new GeneralOrderHistoryVO();
        orderHis.setDdsSalesOrderNo(go.getDdsSalesOrderNo());
        orderHis.setUpdateBy(userName);
        orderHis.setOperatRemark("订单修改操作");
        String rsHis = orderService.insertOrderHistory(orderHis);

        if(rsHis.indexOf("error")!=-1){
            CommonUtil._alertErrorMessage("出错拉！！！","订单数据插入历史表失败："+rsHis);
            return;
        }

        String rs = orderService.updateOrderByReApproval(go,detailList);
     ...
     * @Date 2023/1/9 16:47
     */
    public void setDdsSalesOrderNo(GeneralOrderApprovalVO goa){
        if(null != goa) {
            dataIdField.setText(String.valueOf(goa.getId()));
            sapSalesOrderNoAssembly.setText(goa.getSapSalesOrderNo());
            ddsSalesOrderNoAssembly.setText(goa.getDdsSalesOrderNo());

            String approvalStatus = goa.getApprovalStatus();
            String deliveryStatus = goa.getDeliveryStatus();
            if(null != approvalStatus && !"".equals(approvalStatus)){
                approvalStatusAssembly.getSelectionModel().select(approvalStatus);
            }
            ...
            String salesOrgCode = goa.getSalesOrgCode();
            if("".equals(salesOrgCode)){
                salesOrgAssembly.getSelectionModel().select(null);
            }else{
                List<Map> salesOrg = querySaleOrgList(salesOrgCode);
            ...
            String productGroupCode = goa.getProductGroupCode();
            if("".equals(productGroupCode)){
                productGroupAssembly.getSelectionModel().select(null);
            }else{
                List<Map> productGroup = queryProductGroupList(productGroupCode);
            ...
            String distributionChannel = goa.getDistributionChannelCode();
            if("".equals(distributionChannel)){
                distributionChannelAssembly.getSelectionModel().select(null);
            }else{
                List<Map> distributionChannelList = queryDistributionChannelList(distributionChannel);
            ...
            String customerCode = goa.getCustomerCode();
            if("".equals(customerCode)){
                customerNameAssembly.getSelectionModel().select(null);
                customerCodeAssembly.setText("");
            }else{
            ...
            String songdfCode = goa.getSongdfdm();
            if("".equals(songdfCode)){
                songdfmcAssembly.getSelectionModel().select(null);
                songdfdmAssembly.setText("");
            }else{
            ...
            String saleDeptCode = goa.getSaleDept();//销售部门代码
            if("".equals(saleDeptCode)){
                saleDeptAssembly.getSelectionModel().select(null);
            }else{
                List<Map> saleDeptList = querySalesDeptList(saleDeptCode);
            ...
            String endCustomerCode = goa.getEndCustomerCode();//终端客户代码
            if("".equals(endCustomerCode)){
                endCustomerAssembly.getSelectionModel().select(null);
            }else{
                List<Map> endCustomerList = queryCustomerList(endCustomerCode,salesOrgCode);
            ...
            String salesmanCode = goa.getSalesman();//销售员代码
            if("".equals(salesmanCode)){
                salesmanAssembly.getSelectionModel().select(null);
            }else{
                List<Map> salesmanList = querySalesmanList(salesmanCode);
            ...
            documentDateAssembly.setValue(LocalDate.parse(goa.getDocumentDate()));
            totalAmountAssembly.setText(String.valueOf(goa.getTotalAmount()==null?"0":goa.getTotalAmount()));
            amountNotTaxedAssembly.setText(String.valueOf(goa.getAmountNotTaxed()==null?"0":goa.getAmountNotTaxed()));


            String shippingConditionCode = goa.getShippingPointCode();//装运条件代码
            if("".equals(shippingConditionCode)){
                shippingConditionsAssembly.getSelectionModel().select(null);
            }else{
                List<Map> shippingConditionsList = queryshippingConditionsList(shippingConditionCode);
            ...
            orderSummaryAssembly.setText(goa.getOrderSummary());//唛头/订单摘要
            shippingRemarksAssembly.setText(goa.getShippingRemarks());//发货信息备注

            /*获取明细*/
            List<GeneralOrderDetailVO> orderDetailList = orderService.getOrderListByDdsSalesOrderNo(goa.getDdsSalesOrderNo());
            dataList.removeAll(tableView.getItems());//清空原有明细行
            for(GeneralOrderDetailVO gd : orderDetailList){
                dataList.add(gd);
            }
